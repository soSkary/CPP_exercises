...
FuelType getFuelType()
{
    while (true)
    {
        ...
        {
            case 'g':
            case 'G': return FuelType::gasoline;
            case 'd':
            case 'D': return FuelType::diesel;
            case 'e':
            case 'E': return FuelType::electricity;
            default:
...
double calculatePrice(FuelType fuelType, double distance, double consumption)
{
    switch (fuelType)
    {
        case FuelType::gasoline: return gasPrice * (distance / 100.0) * consumption; 
        case FuelType::diesel: return dieselPrice * (distance / 100.0) * consumption;
        case FuelType::electricity: return electricityPrice * (distance / 100.0) * consumption;
        default:
...
void printCostOfDriving(FuelType fuelType, double distance, double consumption, double price)
{
    ...
    switch (fuelType)
    {
        case FuelType::gasoline: 
            std::cout << " gasoline powered "; break;
        case FuelType::diesel:
            std::cout << " diesel powered "; break;
        case FuelType::electricity:
            std::cout << " electric "; break;
        default:
