...
TEST_CASE("Testing grid_2d")
{
    ...
    SUBCASE("Copy constructor and copy assignment operator")
    {
        grid_2d::grid_2d<int> test2{ test };

        auto it{ test.begin() };
        auto it2{ test2.begin() };

        while (it != test.end() && it2 != test2.end())
        {
            CHECK(*it == *it2);
            ++it;
            ++it2;
        }
        grid_2d::grid_2d<int> test3 = test2;

        it = test.begin();
        it2 = test3.begin();

        while (it != test.end() && it2 != test3.end())
        {
            CHECK(*it == *it2);
            ++it;
            ++it2;
